...
\begin{itemize}
    \item Externí krystalový oscilátor – hotový signál přiveden na vstup MCU, vysoká přesnost a stabilita
    \item Vnitřní RC oscilátor – součástí MCU, levný, rychlý start, horší tolerance a teplotní stabilita
    \item Vnitřní ring oscilátor – lichý počet invertorů, nízká přesnost, používá se při inicializaci
    \item Vnitřní oscilátor s vnějším krystalem/keramickým rezonátorem – přesný a stabilní, rezonátor levnější a méně přesný
\end{itemize}

\subsubsection*{Parametry zdrojů hodin}
\begin{itemize}
    \item Jmenovitý kmitočet, tolerance (odchylka při 25 °C), stabilita (rozsah teplot)
    \item Stárnutí – pomalá změna kmitočtu v čase
    \item Výkonové zatížení, doba náběhu, spotřeba, cena, odolnost proti rušení
\end{itemize}
...
PLL (Phase Locked Loop) vytváří výstupní hodinový kmitočet ze vstupního – umožňuje násobit nebo dělit frekvenci racionálním číslem.\\
...
\begin{itemize}
    \item Oscilátor – referenční kmitočet \(f_1\), dělič \(\div M\)
    \item Fázový komparátor – porovnává \(\frac{f_1}{M}\) se zpětnovazebním signálem \(\frac{f_2}{N}\)
    \item Korekční člen – RC dolní propust filtrující výstup komparátoru
    \item VCO – napětím řízený oscilátor, dělič \(\div N\) ve zpětné vazbě
\end{itemize}
Po uzamčení platí \(\frac{f_1}{M} = \frac{f_2}{N}\), tedy \(f_2 = \frac{N}{M} f_1\).\\

\subsubsection*{Důvody použití PLL}
\begin{itemize}
    \item Rezonátory mají omezený max. kmitočet, procesor potřebuje vyšší
    \item Z hlediska EMC výhodné použít nižší vstupní kmitočet
    \item Z jednoho referenčního kmitočtu lze odvodit více různých hodin pro jádro, sběrnice a periferie
\end{itemize}

\subsection{Watch dog}
Watch dog (COP – Computer Operating Properly) hlídá správný běh programu – chrání před uvíznutím v nekonečné smyčce nebo vykonáváním chybného kódu po rušení.\\

\subsubsection*{Princip}
Časovač, který při přetečení vyvolá RESET mikrokontroléru.\\
Program musí pravidelně provádět pasivaci (krmení psa) zápisem do příslušného registru.\\
Krmení nesmí být pouze v přerušovací rutině – hlavní program by mohl být zaseknutý.\\
Režim okna – pasivace povolena jen ve vymezeném intervalu (např. 75–100 \% periody), příliš brzké i pozdní krmení vyvolá reset.\\

\subsection{UART / SCI}
UART (Universal Asynchronous Receiver-Transmitter) – asynchronní sériové rozhraní, full duplex.\\
Hodinový signál se nepřenáší, obě strany musí mít shodnou přenosovou rychlost a formát rámce.\\
Vodiče: Tx, Rx, GND.\\
Rámec: start bit → datové bity (od LSB) → volitelný paritní bit → stop bit.\\
Přenosová rychlost v baudech: typicky 9600, 19200, 38400, 57600, 115200 Bd.\\
Paritní bit pro detekci chyby 1 bitu – sudá (počet 1 sudý) nebo lichá parita.\\
...
SCI (označení Motoroly pro UART) – rozšíření o 9bitový mód; 9. bit lze použít jako paritní bit nebo adresní značku.\\

\subsection{SPI}
SPI (Serial Peripheral Interface) – synchronní sériové rozhraní master-slave, typicky full duplex.\\
Komunikaci vždy zahajuje master, který generuje hodinový signál.\\
Vodiče: MOSI (master→slave), MISO (slave→master), SCLK (hodiny), SS/CS (výběr slave).\\
...
Master aktivuje SS, v rytmu hodin probíhá výměna dat přes posuvné registry (po 8 taktech 1 bajt).\\
Bez adresace (výběr signálem SS) a bez potvrzování (ACK). Každý slave má obvykle vlastní SS.\\

\subsection{IIC / I2C}
IIC/I2C (Inter-Integrated Circuit) – synchronní sériová sběrnice, pouze 2 vodiče + GND, multi-master.\\
SDA (data), SCL (hodiny). Výstupy jako otevřený kolektor/drain, pull-up na VCC.\\
Dominantní úroveň nízká (zařízení stáhne vodič), recesivní vysoká.\\
...
Start → 7bitová adresa + R/W → ACK → datové bajty + ACK → Stop.\\
ACK: příjemce přidrží SDA nízko. Repeated start: nový start bez stopu (zachování sběrnice).\\
Arbitrace: při kolizi vyhrává ten, kdo vysílá nízkou (dominantní) úroveň.\\
...
Rychlosti: Standard 100 kb/s, Fast 400 kb/s, High speed 3,4 Mb/s, Ultra fast 5 Mb/s.\\
Délka sběrnice omezena kapacitou vodičů (max. 400 pF pro standard mode).\\
